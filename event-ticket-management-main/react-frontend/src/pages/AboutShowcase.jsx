...
export default function AboutShowcase() {
  ...
  const getHostEventUrl = () => {
    if (!user) return '/register?role=ORGANIZER';
    const isOrg = user.role === 'ORGANIZER' || user.role === 'organizer' || user.role === 'ADMIN' || user.role === 'admin';
    if (isOrg) return '/dashboard/event/new';
    return '/register?role=ORGANIZER&notice=' + encodeURIComponent('To host an event, you have to log in as an organizer.');
  };
      ...
      <section className="relative min-h-[80vh] flex items-center justify-center px-4 sm:px-6 lg:px-8 py-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center space-y-8">
          ...
          >
            ...
            <Link to={getHostEventUrl()} className="btn-funky-secondary text-sm px-8 py-4">
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-20">
        ...
        <div className="space-y-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6">
            ...
            <Link to={getHostEventUrl()} className="text-xs font-mono text-[#00f0ff] hover:underline flex items-center gap-1">
      ...
      <section className="py-24 bg-[#ccff00] text-black px-4 sm:px-6 lg:px-8 text-center">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-4">
            ...
            <Link to={getHostEventUrl()} className="px-8 py-4 bg-transparent border-2 border-black text-black font-syne font-black text-sm uppercase hover:bg-black hover:text-white transition-all">
              HOST AN EVENT
            </Link>
          </div>
        </div>
